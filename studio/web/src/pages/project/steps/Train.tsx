...
export default function TrainPage() {
  ...
  /** 传给 SchemaForm 的最终置灰集合 —— 两个来源的并集：
   *  - `busy`（上游 v0.27.0 工作台重构引入）：提交中锁掉**整个**表单；
   *  - `ddpDisabledFields`（本分支）：单卡机器锁掉 ddp_num_processes。
   *
   *  hints 同理合并。busy 那侧的 hint 值是 `false`（上游用它表示「只置灰、不显示
   *  额外说明」），所以要放在前面、让 DDP 的具体文案覆盖它 —— 否则单卡 + 提交中
   *  的组合下用户看到的是一个没有理由的灰字段。 */
  const formDisabledFields = useMemo(() => {
    const all = busy ? Object.keys(schema?.schema.properties ?? {}) : []
    return [...new Set([...all, ...ddpDisabledFields])]
  }, [busy, schema, ddpDisabledFields])

  const formDisabledHints = useMemo(() => {
    if (!busy) return ddpDisabledHints
    const base = Object.fromEntries(
      Object.keys(schema?.schema.properties ?? {}).map((name) => [name, false as const]),
    )
    return { ...base, ...ddpDisabledHints }
  }, [busy, schema, ddpDisabledHints])
            ...
            {configResp === null || !schema ? (
              ...
              <section ref={schemaScrollRef} className="train-config-scroll flex-1 min-w-0 min-h-0 overflow-y-auto pr-1">
                ...
                <SchemaForm
                  ...
                  disabledHints={formDisabledHints}
