...
_pause_if_tty_on_error() {
    local rc=$?
    # Drop the DCU-only temp files (filtered requirements + pin constraints; see
    ...
    if [ -n "$_REQ_CONSTRAINTS" ]; then
        rm -f "$_REQ_CONSTRAINTS"
    fi
...
_prepare_requirements() {
    ...
    # Pin the image-provided HF stack (transformers / safetensors / tokenizers /
    # huggingface_hub) to whatever the vendor image ships. These differ from
    # torch/torchvision: they DO exist on PyPI and pip can install them, but the
    # image's DCU components (vllm, flash_attn) pin exact versions -- a newer copy
    # in venv/ shadows the image one and those components break at runtime.
    # Observed on DTK 26.04: requirements' `transformers>=4.57.0` (no upper bound)
    # pulled 5.16.1 over the image's 4.57.6, dragging huggingface_hub 0.36.0->1.29.0.
    #
    # Generated by the Python helper rather than reimplemented here, so the two
    # install paths (this script's first-run, cli.py's top-up) cannot drift.
    # Best-effort: any failure just skips the constraints (they harden, not gate).
    _REQ_CONSTRAINTS="tmp/constraints.dcu.txt"
    if "$PYTHON" -c 'import sys; sys.path.insert(0, ".");
from pathlib import Path
from studio.services.runtime import torch as t
sys.exit(0 if t.write_dcu_constraints(Path("tmp/constraints.dcu.txt")) else 1)' 2>/dev/null; then
        echo "[studio] setup: pinning image-provided HF packages via $_REQ_CONSTRAINTS"
    else
        _REQ_CONSTRAINTS=""
    fi
    return 0
...
# Echoes `-c <constraints>` when _prepare_requirements produced one, else nothing.
# Kept as a function so both call sites stay one-liners and cannot diverge.
_req_cflag() {
    [ -n "$_REQ_CONSTRAINTS" ] && [ -f "$_REQ_CONSTRAINTS" ] && printf -- '-c %s' "$_REQ_CONSTRAINTS"
    return 0
}
...
if [ -x "venv/bin/python" ]; then
    ...
    if [ -f requirements.txt ]; then
        ...
        _prepare_requirements
        # shellcheck disable=SC2086  # _REQ_CFLAG is intentionally word-split (empty = no flag)
        _pip_install -r "$_REQ_ARG" $(_req_cflag) || { echo "studio.sh: pip install failed" >&2; exit 1; }
    else
...
if [ "$_STALE" = "stale" ]; then
    ...
    if _pip_install -r "$_REQ_ARG" $(_req_cflag); then
        "$PYTHON" tools/check_requirements_changed.py --marker "$_REQ_MARKER" --update-marker >/dev/null 2>&1 || true
