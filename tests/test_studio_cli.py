...
def test_check_torch_cuda_ok_on_dcu(
    ...
    """海光 DTK wheel + 设备可用 → 一行 OK，带卡名与 HIP 版本，级别是 INFO。
    ...
    「设备可用」不该以 WARNING 出现，那会让用户以为环境有问题。
    """
    _install_fake_torch(
        monkeypatch,
        _FakeTorch(
    ...
    with caplog.at_level(logging.DEBUG, logger="studio.cli"):
        cli._check_torch_cuda()
    text = "\n".join(r.getMessage() for r in caplog.records)
    assert "Hygon BW1000" in text
    assert "6.3.42134-abc" in text          # HIP 版本是排错第一手信息
    assert "gfx928" in text
    # 绝不能把 DCU 说成 CPU-only（那会连带弹重装建议）
    assert "CPU-only" not in text
    # 设备可用是好消息，级别必须是 INFO
    assert all(r.levelno <= logging.INFO for r in caplog.records), (
        f"设备可用却报了 WARNING/ERROR: "
...
def test_check_torch_cuda_dcu_unavailable_never_suggests_cuda_reinstall(
    ...
    断言走 caplog 而非 capsys：日志改写后 `_say` 是 logger 的薄包装，CLI 不再
    print（设计 D3）。级别断言 WARNING —— 设备用不了是真问题，不能以 INFO 混在
    叙事行里被用户滑过去。
    """
    _install_fake_torch(
        monkeypatch,
